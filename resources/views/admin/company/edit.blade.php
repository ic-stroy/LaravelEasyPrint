            ...
            <form action="{{route('company.store')}}" class="parsley-examples" method="POST">
                ...
                <div class="row">
                    ...
                    <div class="mb-3 col-6">
                        ...
                        <input type="text" name="delivery_price" class="form-control" required value="{{$company->delivery_price}}"/>
                    </div>
                </div>
                <div class="row">
                    <div class="mb-3 col-6">
                        <label class="form-label">{{__('Region')}}</label>
                        <input type="text" class="form-control" name="region" readonly value="{{$company->address->region??''}}">
                    </div>
                    <div class="mb-3 col-6">
                        <label class="form-label">{{__('District')}}</label>
                        <input type="text" class="form-control" name="district" readonly value="{{$company->address->district??''}}">
                    </div>
                </div>
                <div class="row">
                    <div class="mb-3 col-6">
                        <label class="form-label">{{__('Region')}}</label>
                        <select name="region_id" class="form-control" id="edit_region_id" required>
                            <option disabled selected>{{__('Select region')}}</option>
                        </select>
                    </div>
                    <div class="mb-3 col-6">
                        <label class="form-label">{{__('District')}}</label>
                        <select name="district_id" class="form-control" id="edit_district_id" required>
                ...
                <div class="form-group google-map-lat-lng">
                    ...
                    <div>
                        <span>Lat: <b id="edit_label_latitude">{{$company->address->latitude??''}}</b></span>&nbsp;&nbsp;
                        <span>Lng: <b id="edit_label_longitude">{{$company->address->longitude??''}}</b></span>
                    </div>
                </div>
                <div class="form-group">
                    <div id="map" class="google_maps"></div>
                </div>
                <input type="hidden" name="region" id="edit_region" value="{{$company->address->region??''}}">
                <input type="hidden" name="district" id="edit_district" value="{{$company->address->district??''}}">
                <input type="hidden" name="address_lat" id="edit_address_lat" value="{{$company->address->latitude??''}}">
                <input type="hidden" name="address_long" id="edit_address_long" value="{{$company->address->longitude??''}}">
                <div class="mt-2">
